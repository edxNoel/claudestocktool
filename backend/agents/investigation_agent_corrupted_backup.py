...
class InvestigationAgent:
    ...
    async def _claude_ai_decision(self, findings: List[str], symbol: str) -> List[str]:
        """Use Claude AI to make intelligent investigation decisions"""
        if self.use_claude and self.claude_service:
            try:
                decision_data = await self.claude_service.generate_investigation_decision(findings, symbol)
                return decision_data.get("next_investigations", ["market_analysis"])
            except Exception as e:
                print(f"Claude decision error: {e}")
        
        # Fallback logic
        if len(findings) == 0:
            return ["Start with basic data collection"]
        elif len(findings) == 1:
            return ["Analyze trends and patterns"]
        elif len(findings) == 2:
            return ["Look for external factors"]
        else:
            return ["Cross-validate and conclude"]
    ...
    async def _analyze_price_movement_decision(self, state: InvestigationState, parent_node_id: str) -> str:
        """AI Agent makes autonomous decision about investigation approach using Claude"""
        try:
            price_change = abs(state.price_change_percent or 0)
            direction = "increased" if (state.price_change_percent or 0) > 0 else "decreased"
            
            # Use Claude AI for intelligent investigation strategy
            ...
            if self.use_claude and self.claude_service:
                try:
                    stock_data = {
                        ...
                        "volume": 1000000  # We'll get this from actual data later
                    }
                    ...
                    print(f"Claude AI generated {len(investigation_hypotheses)} hypotheses for {state.symbol}")
                    
                except Exception as e:
                    print(f"Claude analysis error: {e}")
                    # Fallback to basic logic if Claude fails
                    if price_change > 5:
                        investigation_hypotheses = ["Significant market event", "Company-specific catalyst"]
                        parallel_investigations = ["news_analysis", "earnings_check", "market_context"]
                    else:
                        investigation_hypotheses = ["Normal market activity"]
                        parallel_investigations = ["market_context", "volume_analysis"]
            else:
            ...
            # Store Claude's autonomous decisions
    ...
    async def _create_master_inference(self, state: InvestigationState, validation_node_id: str, inference_nodes: List[str]) -> str:
        """Create comprehensive master inference using Claude AI to explain WHY price moved"""
        try:
            # Collect all investigation findings
            all_evidence = []
            investigation_data = {}
            
            # Extract findings from each node for Claude analysis
            for node in state.nodes:
                if node.status == "completed" and node.data:
                    node_data = node.data or {}
                    
                    # Collect evidence for Claude
                    if "sentiment" in str(node_data) or "news" in node.label.lower():
                        ...
                        all_evidence.append(f"News Analysis: {sentiment} sentiment ({score:.0f}% positive)")
                        investigation_data["news"] = node_data
                        ...
                        all_evidence.append(f"Earnings: {performance} estimates by {abs(surprise):.1f}%")
                        investigation_data["earnings"] = node_data
                    
                    elif "market" in str(node_data) or "sector" in node.label.lower():
                        performance = node_data.get("relative_strength", "neutral")
                        all_evidence.append(f"Market: {performance} performance vs sector")
                        investigation_data["market"] = node_data
            ...
            # Use Claude AI to generate intelligent causal analysis
            primary_cause = "Claude AI Analysis"
            ...
            if self.use_claude and self.claude_service:
                try:
                    print(f"Generating Claude AI master inference for {state.symbol}")
                    claude_analysis = await self.claude_service.generate_master_inference(
                        state.symbol, all_evidence, price_data, investigation_data
                    )
                    ...
                    detailed_explanation = claude_analysis.get("detailed_reasoning", "Analysis in progress")
                    cause_confidence = claude_analysis.get("cause_confidence", 0.8)
                    
                    print(f"Claude generated comprehensive analysis for {state.symbol}")
                    
                except Exception as e:
                    print(f"Claude master inference error: {e}")
                    # Fallback only if Claude completely fails
                    executive_summary = f"{state.symbol}: {direction} {magnitude:.1f}% - Analysis needed"
                    primary_cause = "Analysis Required"
                    detailed_explanation = f"The {magnitude:.1f}% price movement requires deeper analysis of market forces and catalysts."
            else:
                # Simple fallback when Claude is not available
                executive_summary = f"{state.symbol}: {direction} {magnitude:.1f}% - Basic analysis"
                primary_cause = "Market Movement Analysis"
                detailed_explanation = f"Stock {direction} by {magnitude:.1f}%. Professional analysis tools unavailable."
            
            # Create final node with Claude AI analysis (or fallback)
            node_id = str(uuid.uuid4())
            node = AgentNode(
                id=node_id,
                type=NodeType.INFERENCE,
                label=f"Master Inference: Why {state.symbol} Price {direction.title()}",
                description=f"Claude AI Analysis: {primary_cause} caused {magnitude:.1f}% {direction}",
                status="completed",
                data={
                    ...
                    "ai_service": "Claude" if self.use_claude else "Fallback",
                    "branches_analyzed": len(inference_nodes),
                    "investigation_quality": "claude_ai" if self.use_claude else "basic"
                },
                parent_id=validation_node_id,
                created_at=datetime.now().isoformat(),
                completed_at=datetime.now().isoformat()
...
"""
    ...
    async def _agent_decision(self, state: InvestigationState):
        """Agent makes autonomous decisions about next investigation steps using Claude AI"""
        
        node_id = str(uuid.uuid4())
        
        # Use Claude AI for decision making
        decisions = await self._claude_ai_decision(state.current_findings, state.symbol)
        
        # Get additional Claude analysis if available
        reasoning = f"Based on {len(state.current_findings)} findings, determining next steps"
        confidence = 0.8
        
        if self.use_claude and self.claude_service:
            try:
                decision_data = await self.claude_service.generate_investigation_decision(state.current_findings, state.symbol)
                reasoning = decision_data.get("reasoning", reasoning)
                confidence = decision_data.get("confidence", confidence)
            except Exception as e:
                print(f"Claude decision analysis error: {e}")
        
        node = AgentNode(
            id=node_id,
            type=NodeType.DECISION,
            label="Claude AI Decision Point",
            description="Claude AI analyzing investigation strategy",
            status="completed",
            data={
                "decisions": decisions,
                "reasoning": reasoning,
